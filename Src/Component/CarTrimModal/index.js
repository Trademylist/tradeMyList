...
const TrimModal = (props) => {
  ...
  const getcartrim = async () => {
    try {
        const data= {
          name: props.selectedModal, 
          type: 'Car'
      }
      //console.log(data)
      ...
      .then(response => {
        if(response.data.data.length > 0){
          setTrimList(response.data.data[0].division)
        }
      })
      .catch(error => {
              ...
              <View style={styles.modalBody}>
                <ScrollView showsVerticalScrollIndicator={false}>
                  ...
                  <View style={styles.HeadrIconContainer}>
                    ...
                    <Text style={{ fontFamily:"Roboto-Bold" , color: '#434343', fontSize: 20, fontWeight: 'bold', textAlign: 'left', alignSelf: 'flex-start', width: Devicewidth / 1.9, marginTop: 19, marginLeft: 20, }}>Trim</Text>
                  </View>
