...
export default class ProductDetails extends Component {
  ...
  getReviewList = async UserId => {
    ...
    if (value !== null) {
      ...
      await axios
        .post('https://trademylist.com:8936/app_seller/get_review', object, {
          headers: {
  ...
  getShare = async () => {
    try {
      ...
      const ProductLink =
        'trade://trademylist.com/product-details/' + productId + '/' + process;
      const ShareLink =
  ...
  getProductDetails = async (productid, process) => {
    ...
    axios
      ...
      .then(response => {
        ...
        console.log(
          '=================////////mypro dtails api call///////////===========',
          response.data.data.product,
        );
        var imageData = [];
        var selectedImage = [];
        var imgzoomData = [];
        var notSelectedImage = [];
        ...
        if (response.data.data.product.cover_thumb == '') {
          if (response.data.data.product.category == 'Jobs') {
            ...
            imgzoomData.push({
              uri: 'https://trademylist.com:8936/services.jpg',
            });
          }
        } else {
        ...
        if (response.data.data.product.image.length > 0) {
          for (var i = 0; i < response.data.data.product.image.length; i++) {
            if (response.data.data.product.image[i] === null) {
              notSelectedImage.push(response.data.data.product.image[i]);
            } else {
              selectedImage.push(response.data.data.product.image[i]);
            }
          }
          console.log('Select image 00000000======', selectedImage);
          selectedImage.map((imgdata, imgindex) => {
            imageData.push(imgdata);
            if (imgdata == '') {
              if (response.data.data.product.category == 'Jobs') {
                imgzoomData.push({
                  uri: 'https://trademylist.com:8936/jobs.jpg',
                });
              } else if (response.data.data.product.category == 'Services') {
                imgzoomData.push({
                  uri: 'https://trademylist.com:8936/services.jpg',
        ...
        Geocoder.from(response.data.data.product.address).then(json => {
          if (imageData[0] == '') {
            if (response.data.data.product.category == 'Jobs') {
      ...
      .catch(error => {
        console.log('errorA', error.response.data);
        this.setState({isLoading: false});
      });
  };
  ...
  getSellerDetails = async userId => {
    try {
      ...
      if (value !== null) {
        await axios
          .get('https://trademylist.com:8936/user/' + userId, {
            headers: {
              'x-access-token': value.token,
            },
          })
          .then(response => {
            ...
            console.log(
              'USER DATA  ===',
              response.data.data.username.charAt(0).toLowerCase(),
            );
          })
          .catch(error => {
            //console.log(error.data)
  ...
  HitPush = async (sellerId, senderId, receiver_id, productId, text) => {
    ...
    if (value !== null) {
      ...
      axios
        .post('https://trademylist.com:8936/app_seller/chat_push', object, {
          headers: {
            'x-access-token': value.token,
          },
        })
  ...
  getAllLikes = async () => {
    ...
    try {
      ...
      if (value !== null) {
        axios
          .get('https://trademylist.com:8936/app_seller/likelist', {
            headers: {
              'x-access-token': value.token,
            },
          })
  ...
  ProductEdit = () => {
    if (this.state.Process === 'general') {
      ...
      this.props.navigation.navigate('commerciallistingDetails', {
        productId: this.state.ProductID,
      });
    }
  };
          ...
          productDetails != '' && (
            <>
              <MySubscriptionModal
                ...
                Process={this.state.Process}></MySubscriptionModal>
              <LoginModal
                modalProps={this.state.loginModal}
                onPressClose={this.closeloginModal}
                getlogin={this.redcLogin}
                navigation={this.props.navigation}></LoginModal>
              <ImageView
              ...
              <ScrollView
                ...
                <View
                  ...
                  <View
                    ...
                    <SliderBox
                      ...
                      sliderBoxHeight={Deviceheight / 3.2}
                      resizeMethod={'resize'}
                      resizeMode={'contain'}
                    ...
                    <View
                      ...
                      <Text
                        ...
                        {this.state.ImageCurrentIndex + 1}/
                ...
                this.state.userDetails !== null ? (
                  <View
                    ...
                    <TouchableOpacity
                      ...
                      {this.state.userDetails &&
                      this.state.userDetails.image !== null ? (
                        <Image
                          source={{uri: this.state.userDetails.image}}
                          style={{
                            height: '100%',
                            width: '100%',
                            borderRadius: 360,
                        ...
                        />
                      ) : this.state.userDetails.username == null ||
                ...
                <View
                  ...
                  {this.state.productDetails.category == 'Freebies' ? (
                    ...
                    <Text
                      ...
                      {this.state.productDetails.category != 'Jobs' &&
                      this.state.productDetails.category != 'Freebies' &&
                      this.state.productDetails.category != 'Services'
                        ? this.state.productDetails.currencyCode == 'INR'
                          ? '₹ '
                          : this.state.productDetails.currencyCode == 'USD'
                          ? '$ '
                          : `${this.state.productDetails.currencyCode} `
                        : null}
                      {this.state.productDetails.category != 'Jobs' &&
                        this.state.productDetails.category != 'Freebies' &&
                        this.state.productDetails.category != 'Services' &&
                        this.state.productDetails.product_price}
                    </Text>
                  )}
                  {this.state.loggedUserId === this.state.sellerId ? (
                    <TouchableOpacity
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        this.state.likesProduct.indexOf(
                          this.props.route.params.productId,
                        ) !== -1
                          ? this.RemoveFavProduct(
                              this.props.route.params.productId,
                            )
                          : this.AddFavProduct(
                              this.props.route.params.productId,
                ...
                <Text
                  ...
                  {this.state.productDetails.product_description}
                </Text>
                {(productDetails.sub_category.length > 0 ||
                  productDetails.sub_category_number.length > 0) && (
                  <View>
                ...
                <Text
                  ...
                  {this.state.productDetails.address}
                </Text>
                <View
                  style={{
                    alignItems: 'center',
              ...
              {this.state.userDetails == '' ? null : (
                <>
                  {this.state.loggedUserId === this.state.sellerId ? null : (
                    <>
                          ...
                          renderItem={({item}) => (
                            <TouchableOpacity
                              style={{
                                ...
                                alignItems: 'center',
                                justifyContent: 'center',
                                backgroundColor: '#363ed9',
                                marginRight: 10,
                                height: 42,
                                paddingHorizontal: 18,
                                marginBottom: 5,
                              }}
                              onPress={() => this.getMessage(item.name)}>
                              <Text
                                style={{
