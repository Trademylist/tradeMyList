...
exports.dummy_product_add = function (req, res) {
  // res.json({ success: false, message: "Document Not Found" });
  upload(req, res, (err) => {
    if (err) {
      res.json({ success: false, message: "Document Not Found" });
    ...
    for (let i = 1; i < array.length - 1; i++) {
      ...
      for (let j in headers) {
        ...
        // d2codeConvert = [d2codeConvert[1], d2codeConvert[3], d2codeConvert[5]];

        let fynaleData = properties[j].split(d2codeConvert[1]).join("");
        let imgArr = [];
        if(d2codeConvert[5] && d2codeConvert[5].length>0){for (const iterator of d2codeConvert[5].split(',')) {
    ...
    Product.insertMany(result).then(function () {
    res.json({ success: true, data: result, message: "Product's successfully created" });  // Success
