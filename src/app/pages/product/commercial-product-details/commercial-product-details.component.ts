  ...
  @Component({
    selector: 'app-commercial-product-details',
    templateUrl: './commercial-product-details.component.html',
    styleUrls: ['./commercial-product-details.component.scss']
  })
  export class CommercialProductDetailsComponent implements OnInit {
    public shareLink: string = window.location.href; //this.router.url;
    ...
    getItem() {
      this.productDetails = {};
    this.apiService.get('app_user/freebies', this.itemId).subscribe(resp => {
      if (resp.success) {
        let respData = resp.data.product;
        this.productGalleryFormat(respData.image);
        let today = moment();
        ...
        if (this.productDetails.category == "Van & Trucks") {
        ...
        } else if (this.productDetails.category == "Property") {
          this.productDetails.furnishing = subCategory[0].value ? subCategory[0].value : '';
          this.productDetails.construction_status = subCategory[1].value ? subCategory[1].value : '';
          this.productDetails.listed_by = subCategory[2].value ? subCategory[2].value : '';
          this.productDetails.car_parking = subCategoryNumber[0].value ? subCategoryNumber[0].value : '';
          this.productDetails.super_builtup_area = subCategoryNumber[1].value ? subCategoryNumber[1].value : '';
          this.productDetails.carpet_area = subCategoryNumber[2].value ? subCategoryNumber[2].value : '';
          this.productDetails.maintenance = subCategoryNumber[3].value ? subCategoryNumber[3].value : '';
          this.productDetails.washrooms = subCategoryNumber[4].value ? subCategoryNumber[4].value : '';
        }
        this.loadMap();
      } else {
  ...
  getSellerDetails() {
    ...
    showChatOption() {
      ...
      return (len && this.loggedUser != null ? (this.loggedUser.userid == this.seller_id ? false : true) : false);
    }
  ...
  editProduct() {
    let navigationExtras: NavigationExtras = {
      queryParams: {
        ...
        "product_id": this.productDetails._id,
        // "categoryDetails": JSON.stringify(this.getCommercialCategory),
        // "categoryImageUrl": JSON.stringify(this.getCommercialCategoryImageUrl)
      }
    };
  ...
  checkLoggedUser() {
    ...
    productGalleryFormat(productImages){
